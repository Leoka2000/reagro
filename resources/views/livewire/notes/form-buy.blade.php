...
<div>
    ...
    <div>
        ...
        <header class='flex flex-col items-center justify-start gap-3 sm:flex-row dark:text-gray-300'>
            ...
            <x-card title="Tipo do produto" class=" bg-teal-500/10 from-white  dark:bg-teal-300/20 dark:bg-gradient-to-bl bg-gradient-to-bl dark:from-gray-900  via-transparent dark:ring-1 dark:ring-inset dark:ring-white/5 rounded-lg shadow-lg  dark:shadow-none flex  transition-all duration-250 focus:outline focus:outline-2 focus:stroke-teal-500">
                <x-slot name="action">
            ...
            <x-card title="Cidade" class=" bg-teal-500/10 from-white  dark:bg-teal-300/20 dark:bg-gradient-to-bl bg-gradient-to-bl dark:from-gray-900  via-transparent dark:ring-1 dark:ring-inset dark:ring-white/5 rounded-lg shadow-lg  dark:shadow-none flex  transition-all duration-250 focus:outline focus:outline-2 focus:stroke-teal-500">
                <x-slot name="action">
                    <span class="rounded-full focus:outline-none focus:ring-2 focus:ring-indigo-600">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 3c2.755 0 5.455.232 8.083.678.533.09.917.556.917 1.096v1.044a2.25 2.25 0 0 1-.659 1.591l-5.432 5.432a2.25 2.25 0 0 0-.659 1.591v2.927a2.25 2.25 0 0 1-1.244 2.013L9.75 21v-6.568a2.25 2.25 0 0 0-.659-1.591L3.659 7.409A2.25 2.25 0 0 1 3 5.818V4.774c0-.54.384-1.006.917-1.096A48.32 48.32 0 0 1 12 3Z" />
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <x-card class='relative bg-teal-200/20 from-white  dark:bg-teal-300/20 dark:bg-gradient-to-bl bg-gradient-to-bl dark:from-gray-900  via-transparent dark:ring-1 dark:ring-inset dark:ring-white/5 rounded-lg shadow-lg  dark:shadow-none flex  transition-all duration-250 focus:outline focus:outline-2 focus:stroke-teal-500' wire:key='{{ $note->id }}'>
                    ...
                    <main class='flex flex-col justify-between h-full '>
                        <div>
                            <x-badge rounded sm positive class='absolute text-gray-700 h-7 top-10 right-10 positive' icon='exclamation-circle' label="Venda imediata" />
                            ...
                            <header>
                                <div class="block">
                                    ...
                                    <div class='flex items-center justify-center md:max-w-96 md:max-h-96'>
                                        ...
                                        <img class='object-cover w-full rounded-md h-80 bg-slate-300' src="{{ asset('storage/' . $firstImage) }}" alt="Image" title="product image" />
                                    </div>
                                </div>
                            </header>
                            <div class='mt-5'>
                                <div class='pb-4'>
                                    ...
                                    <div class='pt-2 pb-4'>
                                        <x-badge class='h-7' rounded positive outline label="R$ {{ $note->price }}" />
                        ...
                        <footer>
                            <div class='flex items-center justify-end gap-2'>
                                @can('update', $note)
                                <!-- Update button -->
                                <x-button.circle href="{{ route('notes.edit', $note) }}" sm green outline icon="pencil-alt"></x-button.circle>
                                @endcan
                ...
                <x-modal wire:model="showModal" class="" title="Simple Modal">
                    <div class='flex flex-col h-auto gap-2 p-12 bg-white dark:bg-gray-900 dark:text-gray-300 w-96 rounded-xl '>
                        <p class='mb-4 text-gray-700 dark:text-gray-200 dark:bg-gray-900 sm:text-base'>Tem certeza que dejesas
                            deletar o anúncio?</p>
                        <x-button rounded icon='arrow-left' wire:click="closeModal">Sair</x-button>
                        <x-button rounded flat negative spinner='delete' outline icon='trash' wire:click="delete('{{ $noteToDelete->id }}')">Continuar</x-button>
                    </div>
